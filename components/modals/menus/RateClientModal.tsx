...
import { rateClient } from '@/store/slices/clientSlice';
...
const RateClientModal: React.FC<Props> = ({ visible, onClose, bookingId }) => {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const resultAction: any = await dispatch(rateClient({ bookingId: bookingId!, rating, review }));
