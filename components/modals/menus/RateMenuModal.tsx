...
const RateMenuModal: React.FC<Props> = ({ visible, onClose, menuId }) => {
  ...
  const submit = async () => {
    ...
    console.log('RateMenuModal:menuId', menuId, 'rating', rating, 'review', review);
    if (rating < 1 || rating > 5) return Toast.show({ type: 'error', text1: 'Rating must be between 1 and 5' });

    setLoading(true);
    try {
      const resultAction: any = await dispatch(rateMenu({ menuId: menuId, rating, review }));
      console.log('RateMenuModal:rateMenu resultAction', resultAction);
      ...
      <View style={styles.overlay}>
        <View style={styles.container}>
          <View style={styles.header}>
            <Text style={styles.title}>Rate menu</Text>
